...
export default function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const mailtoLink = `mailto:snehashish@example.com?subject=${encodeURIComponent(formData.subject)}&body=${encodeURIComponent(`Name: ${formData.name}\nEmail: ${formData.email}\n\nMessage:\n${formData.message}`)}`
  ...
  const socialLinks = [
    { icon: Mail, label: "Email", href: "mailto:snehashish@example.com", color: "hover:text-primary" },
    ...
    <section id="contact" className="py-20 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        <motion.div ref={ref} variants={containerVariants} initial="hidden" animate={inView ? "visible" : "hidden"}>
          ...
          <motion.div variants={itemVariants} className="p-8 rounded-lg bg-card border border-border mb-8">
